...
export class NetworkService {
      ...
      try {
        ...
        if (switchError.code === 4902) {
          try {
            ...
            this._messageService.showMessage("Switch Networks: " + addError.message, 3000);
          }
        } else {
          this._messageService.showMessage("Switch Networks: " + switchError.message, 3000);
        }
      }
    } else if (type === "binance" && this.win.BinanceChain) {
      // TODO: investigate switch or add network with binance chain wallet

      this._messageService.showMessage("Switch Networks: Please connect your wallet to " + supported_network.chainName + " network");
    } else {
      this._messageService.showMessage("Switch Networks: Please connect your wallet to " + supported_network.chainName + " network");
    }
  }
  ...
  public async addToken(token: Token): Promise<void> {
    ...
    if (type === "metamask" && this.win.ethereum) {
      ...
      this._messageService.showMessage("Add token: Please add " + token.tokenName + " token to your wallet");
    }
  }
}
